...
Designed to fit on a standard **3x3 Advanced Monitor** (approx. 39x19 text scale 1.0). Tested in ATM10 7.3 (1.21.1, AE2 19.2.17 and Advanced Peripherals version 0.7.62b).
...
1. Put the files into your computer's directory: `../world/computercraft/computer/#/`.
2. Connect your **ME Bridge** and **Advanced Monitor** and turn the computer on.
3. Configure the monitor side at the top of each script if they don't auto-detect:
   ```lua
   ...
   ```
4. Run any monitor by invoking the filename:
   ```bash
   disk
   ```
5. To run a script on startup, update `startup.lua`:
   ```lua
   shell.run("disk")
   ```
